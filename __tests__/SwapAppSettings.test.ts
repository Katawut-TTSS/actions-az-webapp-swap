...
test('test SwapAppSettings slotSettings inherit should preserve false from Azure setting', () => {
  const sharedConfig = {
    ...globalConfig,
    defaultSensitive: DefaultSensitiveEnum.false,
    defaultSlotSetting: DefaultSlotSettingEnum.inherit,
...
test('test SwapAppSettings function_app should force non-slot settings to false', () => {
  const sharedConfig = {
    ...globalConfig,
  ...
  expect(new SwapAppSettings(swapAppService).fullfill([], 'production')).toStrictEqual({
    ...sharedConfig,
    connectionStrings: [],
    appSettings: [
      {
